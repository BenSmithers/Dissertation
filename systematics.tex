...
Such is the case for the scattering and absorption lengths as a function of depth in IceCube, which are influenced by many forms of optical impurity. 
To address this, a new method for efficiently determining the effects of these parameters was developed for IceCube analyses: SnowStorm\footnote{The name SnowStorm reflects the idea that each event in the sample is distinct, like each snowflake in a snowstorm~\cite{Aartsen_2019_snow}}. 
It is discussed at-length in Reference~\cite{Aartsen_2019_snow}.
Several Snowstorm implementations are used in this analysis, and so it is introduced in the section that follows. 

Snowstorm relies on two underlying assumptions which are generally the same for most commonly used methods of systematic uncertainty estimation.
First, it assumes the effects of systematic uncertainty are sufficiently small such as to be treated perturbatively. 
Second, it assumes that the Monte Carlo statistical uncertainty is small compared to the data.

We consider some set of parameters $\vec{\eta}$, called ``nuisance parameters,'' which are imperfectly known and yet contribute to the predicted final event rates. 
We wish to, given some choice of $\vec{\eta}$, calculate a predicted event rate $\psi_{\vec{\rho}, \vec{\eta}}(E_{\alpha})$, where a central prediction is denoted $\psi_{\vec{\rho}, \vec{0}}(E_{\alpha})$.

A SnowStorm Monte Carlo ensemble consists of a single sample where every constituent event draws a unique set of nuisance parameters $\vec{\eta}$ from prior constraints. 
...
A SnowStorm ensemble's distinction from classical MC methods for systematic uncertainty evaluation is shown in Figure~\ref{fig:snostorm_grad}.

\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{./figures/wacky_snowstorm.png}
    \caption{On the left: an illustration of the difference between the classical systematic uncertainty implementation of discrete Monte Carlo samples (red squares) and the single SnowStorm ensemble (blue dots). On the right: }\label{fig:snostorm_grad}
\end{figure}

We first require that tje prior function describing the nuisance parameters $\vec{\eta}$ is normalized and symmetric for each $\eta_{i}$, such that 
\begin{align}
...
\end{equation}
The effects of these nuisance parameters on analysis space can be calculted, in principle, if we know the gradients about the central distribution:
...
From the single SnowStorm ensemble, the method used for gradient extraction involves the sub-division of the full ensemble by cutting it along different directions.
For example, to extract the the $\eta_{i}$ gradient, we divide it along the $\eta_{i}=0$ plane into sub-samples $\psi_{\vec{\rho}}^{i+}$ and $\psi_{\vec{\rho}}^{i-}$, which each encode the predictions:
\begin{align} 
...
In one, a sample was generated while allowing the amplitudes and phases, for a Fourier series describing the depth-dependence of the absorption length and scattering length, to vary amounts shown in Figure~\ref{fig:ice_param_widths}. 
A full description of this and the process of extracting these widths from calibration is described in detail in Subsection~\ref{sec:bulk}, and the priors widths used in the generation of this SnowStorm MC sample are shown in Table~\ref{table:snowstorm2}. 
...
PISA~\cite{pisa}, the fitting framework used for this analysis, is incompatible with correlated priors due to the details of its construction. 
...
If we assume the distribution is jointly normally distributed, then the probability density function defining the propability of a set of nuisance parameters $\vec{y}$ being the ``true'' values is given by
\begin{equation}
...
A table showing parameterization of all included systematic uncertainties, their central values, their prior widths, the event morphologies they apply to, and notes about the implementation are included in Table~\ref{table:nutrition}.
